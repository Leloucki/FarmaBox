    ...
    var MERCADO_JS = {
		...
		mercado_toggle_slide_menu: function() {
    		if($(".widget .has-child-cate").length > 0){
    			$(document).on('click',".widget .has-child-cate .toggle-control, .catelinkProdutos", function(el){
    				el.preventDefault();
    				var _this = $(this);
		...
		mercado_price_quantity: function(){
			if($(".quantity-input").length > 0){
				$(".quantity-input").on('click', '.btn', function(event) {
					event.preventDefault();
					var _this = $(this),
						_input = _this.siblings('input[id=quantidade]'),
						_current_value = _this.siblings('input[id=quantidade]').val(),
						_max_value = _this.siblings('input[id=quantidade]').attr('data-max'),
						_price_value = getMoney(_this.closest('li').find('.product-price p').text()),
						_sub_total_el = _this.closest('li').find('.sub-total p'),
						_sub_total = getMoney(_sub_total_el.text())
						;
						//console.log(_price_value);
					if(_this.hasClass('btn-reduce')){
						if (parseInt(_current_value, 10) > 1) {
							_input.val(parseInt(_current_value, 10) - 1);
							_sub_total = getMoney(_sub_total_el.text());
							_sub_total = _sub_total - _price_value;
							_sub_total_el.text(formatReal(_sub_total));
						}
					}else {
...
$(document).on('click', '#opPerfil' ,function(e){
	switch ($(this).attr('value')) {
		case 'conta':
			$('#meusProdutos').css('display', 'none');
			$('#pagamentoPerfil').css('display', 'none');
			$('#minhaConta').css('display', 'block');
			$('.box-title').text('minha conta');
			break;
		
		case 'produtos':			
			$('#minhaConta').css('display', 'none');
			$('#pagamentoPerfil').css('display', 'none');
			$('#meusProdutos').css('display', 'block');
			$('.box-title').text('meus produtos');
			break;

		case 'pagamento':			
			$('#minhaConta').css('display', 'none');
			$('#meusProdutos').css('display', 'none');
			$('#pagamentoPerfil').css('display', 'block');
			$('.box-title').text('Pagamento');
			break;
...
$('.addProduto').on('click', function(){
	...
	$.ajax({
		url: 'produtos/inserirClienteProduto',
		method: 'POST',
		data: {
...
function setSumario(){
	//subTotal = getMoney($('#subtotalPerfil').text());
	subTotal = 0;
	produtosValor = $('.sub-total p').text().split('R$').slice(1);
	
	for(var i in produtosValor){
		produtosValor[i] = getMoney(produtosValor[i]);
		subTotal += produtosValor[i];
	}
